...
def generate_grid(screen, polygon_vertices):
    # Utworzenie siatki ulic
    grid = []
    min_x = int(min(polygon_vertices, key=lambda v: v[0])[0])
    max_x = int(max(polygon_vertices, key=lambda v: v[0])[0])
    min_y = int(min(polygon_vertices, key=lambda v: v[1])[1])
    max_y = int(max(polygon_vertices, key=lambda v: v[1])[1])
...
def draw_polygon(screen, polygon_vertices):
    ...
    pygame.draw.polygon(screen, GRAY, polygon_vertices, 3)
